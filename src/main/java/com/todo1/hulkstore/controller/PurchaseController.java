...
public class PurchaseController {
    ...
    @GetMapping("/findall")
    private ResponseEntity<List<Purchase>> findAllPurchases() {
     ...
     @PostMapping("/save")
    @ResponseBody
    private ResponseEntity<Purchase> saveProduct(@RequestBody Purchase purchase) {
        Purchase obj = purchaseService.create(purchase);

        try {
            return ResponseEntity.created(new URI("/api/hulkstore/save" + obj.getCompraId()))
                .body(obj);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).build();
        }
    ...
    private ResponseEntity<Optional<Purchase>> findById(@PathVariable Long id) {
